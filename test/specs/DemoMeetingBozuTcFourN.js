...
describe("interaction with web element", function () {
  it("enter the value in the field", async () => {
    ...
    const bozuStoryPeers = $$("//div[contains(text(),'BOZU STORY')]/../../..//label[contains(@class, 'peer-name')]");
    // let listofpeers = []
    // for (let i = 0; i < (await bozuStoryPeers).length; i++) {
    //   const peer = await bozuStoryPeers[i].getText();
    ...
    if(listofBozuStoryPeers.includes("Atul") && listofBozuStoryPeers.includes("Bhavika")){
      console.log("PASS");
    ...
    const bozuTechStoryPeers = $$("//div[contains(text(),'BOZU TECH STORY')]/../../..//label[contains(@class, 'peer-name')]");
    let listofBozuTechStoryPeers = await bozuTechStoryPeers.map(async peer => await peer.getText())
    console.log("&&&&&&&& ",listofBozuTechStoryPeers);
    if(listofBozuTechStoryPeers.includes("Jaydeep") && listofBozuTechStoryPeers.includes("Unnati")){
      console.log("PASS");
